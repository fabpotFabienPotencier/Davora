...
export default function Signup() {
  ...
  const handleGitHubLogin = () => {
    if (!githubClientId) {
      setError('GitHub sign-up is not configured yet on the server.');
      return;
    }
    ...
    if (code) {
      ...
      fetch((process.env.NEXT_PUBLIC_API_URL || 'https://api.davora.xyz') + '/api/auth/github', {
        ...
        .then(async (res) => {
          ...
          if (!res.ok) throw new Error(data.detail || 'GitHub sign-up failed');
            ...
            {isEmailSent ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '8px' }}>
                ...
                >
                  ...
                  {isGitHubLoading ? 'Connecting to GitHub...' : 'Sign up with GitHub'}
                </button>
